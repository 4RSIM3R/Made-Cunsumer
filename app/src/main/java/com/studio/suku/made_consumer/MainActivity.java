...
import static com.studio.suku.made_consumer.LocalDb.DatabaseContract.Entry.CONTENT_URI;
...
public class MainActivity extends AppCompatActivity implements LoadFavoriteCallback {
    ...
    public void postExecute(Cursor favorites) {
        ...
        if (list.size() > 0) {
            consumerAdapter.setListFavorites(list);
        } else {
            Toast.makeText(this, "Tidak Ada data saat ini", Toast.LENGTH_SHORT).show();
            consumerAdapter.setListFavorites(new ArrayList<FavoriteItem>());
        }
    }
